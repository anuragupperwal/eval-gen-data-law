...
_QA_RE = re.compile(r"Q\d+\..+?Answer:\s*[A-F1-6]", re.S)

# PROMPT_TMPL = """You are a law-domain question setter.
# Using ONLY the passages below, write {n} {diff_tag}-level multiple-choice questions (A-F) and provide the correct answer.
# Each question must test understanding of *{leaf_label}*.
...
Goal: Using ONLY the passages below, write {n} {diff_tag}-level multiple-choice questions (1-6) with exactly one correct answer per question. 
...
class RAGGenerator:
    ...
    def generate_for_bundle(self, bundle_path: Path, n=3):
        ...
        difficulty = bundle.get("difficulty", "M")        
